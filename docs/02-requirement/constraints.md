...
| CON-77 | Trạng thái mong muốn của mỗi môi trường nằm trong git, dưới dạng **một nhánh dài hạn cho mỗi môi trường** (`dev` → `staging` → `production`). Triển khai là merge pull request vào nhánh của môi trường đó, không phải một lệnh mệnh lệnh phát ra từ pipeline. *(Trước 2026-09-18 ràng buộc này ghi: "Triển khai `production` là một commit đổi thẻ ảnh trong overlay" — xem [ADR-0015](../adr/0015-branch-per-environment.md) về lý do đổi.)* | [ADR-0015](../adr/0015-branch-per-environment.md) |
